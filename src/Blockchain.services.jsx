...
window.web3 = new Web3(window.web3.currentProvider); //returns metamask

const getEtheriumContract = async () => {
  const web3 = window.web3;
  const networkId = await web3.eth.net.getId(); //gettignt the network Id
  const networkData = abi.networks[networkId];

  if (networkData) {
    const contract = new web3.eth.Contract(abi.abi, networkData.address); //web3.js approach of connecting to the SC.
    return contract;
  } else {
    return null;
  }
