...
//
    ...
    function payToMint(
        ...
        minted.push(TransactionStruct(supply,msg.sender,
        salesPrice, title, description, metadataURI, block.timestamp));
...
//
    function payToBuy(uint256 id) external payable {
        require(msg.value >= minted[id -1].cost, "Ether is too low for purchase!"); //WRONG
        require(msg.sender != minted[id -1].owner, "Operation not allowed!");
        
        uint256 royalityFee = (msg.value * royaltyFee) / 100;
        payTo(artist, royalityFee);
        payTo(minted[id - 1 ].owner, (msg.value - royalityFee));
...
//
    function changePrice(uint256 id, uint256 newPrice) external returns (bool) {
        require(newPrice > 0 ether, "Either too low");
        require(msg.sender == minted[id - 1].owner, "Operation not allowed");
